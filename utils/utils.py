...
def visualize_attention_map(img: torch.Tensor,
    ...
    Args:
        img (torch.Tensor): Input image tensor of shape (2, H, W) for MATE TPC data.
        ...
        attn_weights (torch.Tensor): Real attention weights from model of shape (num_heads, seq_len, seq_len).
                                     seq_len = num_patches + 1 (physics token + spatial patches)
    ...
    img_np = img[0].detach().cpu().numpy()  # Shape: (H, W)
    ...
    if attn_weights.dim() == 3:
        ...
        # attn_weights[:, 0, 1:] gives attention from physics token to spatial patches
        spatial_attn = attn_weights[:, 0, 1:].detach().cpu().numpy()  # Shape: (num_heads, num_patches)
    else:
    ...
    avg_attn = spatial_attn.mean(axis=0)  # Shape: (num_patches,)

    # Calculate grid size based on image dimensions
    # After ResNet with 3 stride-2 layers: H/8 x W/8
    grid_h = img_height // 8
    grid_w = img_width // 8
...
if __name__ == "__main__":
    ...
    # Create a dummy image (2, 80, 48) for Y-Z projection
    dummy_img = torch.randn(2, 80, 48)

    # Calculate expected sequence length
    # For 80x48: feature map = 10x6 = 60 patches + 1 physics token = 61
    num_patches = (80 // 8) * (48 // 8)
    seq_len = num_patches + 1
    
    # Create realistic attention weights (8 heads, 61 tokens, 61 tokens)
    dummy_attn = torch.softmax(torch.randn(8, seq_len, seq_len), dim=-1)
